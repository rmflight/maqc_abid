...
procAvgDat <- function(affyDat, affySamp, agilDat, agilSamp){
	...
	sdA <- sd(distA)
	sdB <- sd(distB)
	return(list(A=list(affy=affy.A, agil=agil.A, reg=rma.A, proj=projA, dist=distA, sd=sdA),
							B=list(affy=affy.B, agil=agil.B, reg=rma.B, proj=projB, dist=distB, sd=sdB)))
}
```
...
```{r reduce2common}
affyAvgEnt <- affyAvgEnt[rownames(affyAvgEnt) %in% rownames(agilAvgEnt),]
agilAvgEnt <- agilAvgEnt[rownames(agilAvgEnt) %in% rownames(affyAvgEnt),]

affyAvgEnt <- affyAvgEnt[order(rownames(affyAvgEnt)),]
agilAvgEnt <- agilAvgEnt[order(rownames(agilAvgEnt)),]

all.equal(rownames(affyAvgEnt), rownames(agilAvgEnt))
```
...
agilAvgEnt.A <- apply(agilAvgEnt[,pDat$Sample == "A"], 1, mean)
agilAvgEnt.B <- apply(agilAvgEnt[,pDat$Sample == "B"], 1, mean)

rma.A <- gx.rma(affyAvgEnt.A, agilAvgEnt.A, log=FALSE, x1lab="affy A", x2lab="agilent A")

projA <- affyAvgEnt.A * rma.A$a1 + rma.A$a0
distA <- sqrt((affyAvgEnt.A - projA)^2 + (agilAvgEnt.A - projA)^2)

rma.B <- gx.rma(affyAvgEnt.B, agilAvgEnt.B, log=FALSE, x1lab="affy B", x2lab="agilent B")
projB <- affyAvgEnt.B * rma.B$a1 + rma.B$a0
distB <- sqrt((affyAvgEnt.B - projB)^2 + (agilAvgEnt.B - projB)^2)

# test the function
affyAvgEnt <- calcAverageGene(affyE1, "ID", "ENTREZ_GENE_ID", affyEset)
agilAvgEnt <- calcAverageGene(agilEntMap, "ID", "GENE", agilEset)
entRes <- procAvgDat(affyAvgEnt, pDat$Sample, agilAvgEnt, pDat$Sample)
```
